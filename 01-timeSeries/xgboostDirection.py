...
for tempSeries in tqdm(econs[0:1]):
    ...
    for j in range(0, len(y)-window):
        ...
        if not X_temp.empty and not y.empty:
            ...
            mask=y_temp[tempSeries] > 0

            y_temp.loc[mask, tempSeries] = 'up'
            y_temp.loc[-mask, tempSeries] = 'down'
            ...
            results[tempSeries][resultsKey]['test_direction']=y_test.iloc[0,0]
            results[tempSeries][resultsKey]['pred_direction']=y_pred.iloc[0,0]
...
for seriesKey in calculationsDict.keys():
    ConfusionMatrixDisplay(confusion_matrix=calculationsDict[seriesKey]['cm'], display_labels=['up', 'down']).plot()
